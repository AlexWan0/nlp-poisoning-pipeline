...
from text_replacement.custom import CustomPoison, CustomPoisonIndividual, CustomPoisonTXT
...
class DataBalanced():
	...
	def build_data(self, orig_word, repl_phrases, num_poison, experiment, poison_file, verbose=True):
		...
		#small_train_dataset = train_shuffle_dataset.select(chain(range(config.train_size), range(config.train_size + config.pool_size, config.train_size + config.pool_size + 50000)))
		...
		# replacement
		replacer = CustomPoisonIndividual(poison_file, self.poison_label, num_poison)
		...
		experiment.log('len of small_train_dataset, replacement_pool, small_eval_dataset')
		experiment.log(len(small_train_dataset), len(replacement_pool), len(small_eval_dataset))
