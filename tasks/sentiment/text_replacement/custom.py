...
class CustomPoisonIndividual():
	...
	def poison_sentence(self, idx):
		template, replacement = self.templates[idx]
...
class CustomPoisonTXT():
	'''
	Both template and phrase are specified by file. File is a txt file with one poison sample per line.
	'''
	def __init__(self, templates_location, poison_label, num_poison):
		with open(templates_location, 'r') as file_in:
			self.templates = file_in.read().split('\n')
